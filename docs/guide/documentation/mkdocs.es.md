...
El árbol `nav` es la barra lateral, en orden. Dos convenciones que merece la pena
copiar:

- Una ruta suelta como **primera** entrada de una sección (`arena/index.md`) hace
...
convención es un **sufijo en el nombre del archivo**: `page.md` es el inglés y
`page.es.md` su gemelo en español, uno al lado del otro en la misma carpeta.
...
- El idioma **por defecto** se construye en la raíz del sitio; los demás bajo
  `/es/`.
- El `nav` se declara **una sola vez**, con rutas en inglés. El plugin sustituye
  el archivo traducido cuando existe y recae en la página inglesa cuando no — así
  que un sitio traducido a medias sigue construyéndose y sigue navegándose.
- `nav_translations` traduce las etiquetas de la barra lateral, que están en
  `mkdocs.yml` y no dentro de ninguna página.
