...
Consulta [El torneo](tournament.md) para saber cómo funcionan los tiempos y las
descalificaciones, y [El marcador](scoreboard.md) para el contenido del archivo de
resultados.
...
Este sitio es MkDocs. Se publica en inglés y español desde un único árbol:

```bash
pip install -e ".[docs]"
mkdocs serve          # http://127.0.0.1:8000
mkdocs build --strict # exactamente lo que ejecuta CI
```

## Adónde ir después

- [Reglas del juego](rules.md) — contra qué estás programando en realidad.
- [API de jugador](player-api.md) — la interfaz que implementa toda IA.
- [Enviar un jugador](submit-a-player.md) — mete el tuyo en el torneo.
