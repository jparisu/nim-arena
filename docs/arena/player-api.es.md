...
[`nimarena.game.is_legal`](code-structure.md). En el torneo, devolver una jugada
...
- [Reglas del juego](rules.md) — la estrategia ganadora que ningún jugador
  incluido implementa.
- [El torneo](tournament.md) — quien te llama, en detalle.

## La clase en sí (autogenerada)

!!! info "Esta referencia se genera del código fuente y está en inglés"
    El bloque siguiente sale directamente de los docstrings del paquete.
